"use client";

import { useRouter } from "next/navigation";
import { MediaKind, PublishStatus } from "@prisma/client";
import { ResourceForm, type FieldDescriptor } from "@/components/dashboard/resource-form";
import { ImagePicker } from "@/components/dashboard/image-picker";
import {
  mediaVideoSchema,
  type MediaVideoInput,
} from "@/lib/validators/media";
import { createVideoPost, updateVideoPost } from "@/server/actions/media";
import type { ActionResult } from "@/server/actions/types";

const STATUS_OPTIONS = Object.values(PublishStatus).map((s) => ({
  label: s,
  value: s,
}));

type Props = {
  mode: "create" | "edit";
  postId?: string;
  defaults: MediaVideoInput;
};

export function VideoPostForm({ mode, postId, defaults }: Props) {
  const router = useRouter();

  const fields: FieldDescriptor<MediaVideoInput>[] = [
    { name: "title", label: "Title", tab: "en" },
    { name: "titleUr", label: "Title (Urdu)", tab: "ur" },
    { name: "slug", label: "Slug", tab: "all" },
    {
      name: "summary",
      label: "Summary",
      type: "textarea",
      tab: "en",
      rows: 3,
    },
    {
      name: "coverImage",
      label: "Thumbnail",
      tab: "all",
      render: ({ value, onChange }) => (
        <ImagePicker
          value={typeof value === "string" ? value : null}
          onChange={(url) => onChange(url)}
        />
      ),
    },
    {
      name: "videoUrl",
      label: "YouTube or Vimeo URL",
      type: "url",
      tab: "all",
      description: "Public pages embed via youtube-nocookie or Vimeo player.",
    },
    { name: "publishedAt", label: "Published date", type: "date", tab: "all" },
    {
      name: "status",
      label: "Status",
      type: "select",
      options: STATUS_OPTIONS,
      tab: "all",
    },
  ];

  return (
    <ResourceForm
      schema={mediaVideoSchema}
      fields={fields}
      defaultValues={{ ...defaults, kind: MediaKind.INTERVIEW }}
      showLanguageTabs
      autoSlug
      submitLabel={mode === "create" ? "Create video" : "Save video"}
      preview={(values) => (
        <div className="space-y-2">
          <p className="font-mono text-[10px] font-medium uppercase tracking-wider text-resin">
            Video
          </p>
          <p className="font-sans text-sm font-medium text-bark">
            {values.title || "Untitled video"}
          </p>
          {values.videoUrl ? (
            <p className="truncate font-mono text-[11px] text-deodar">
              {String(values.videoUrl)}
            </p>
          ) : null}
        </div>
      )}
      onSubmit={async (values): Promise<ActionResult> => {
        const payload = { ...values, kind: MediaKind.INTERVIEW };
        const result =
          mode === "create"
            ? await createVideoPost(payload)
            : await updateVideoPost(postId!, payload);
        if (result.ok) {
          router.push("/dashboard/media");
          router.refresh();
        }
        return result;
      }}
      onCancel={() => router.push("/dashboard/media")}
    />
  );
}
