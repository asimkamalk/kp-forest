  ...
  .superRefine((val, ctx) => {
    ...
    try {
      ...
      if (parsed === null || typeof parsed !== "object") {
        ctx.addIssue({
          code: "custom",
          message: "mapGeoJson must be a JSON object or FeatureCollection",
        });
      }
    } catch {
      ctx.addIssue({
        code: "custom",
        message: "mapGeoJson must be valid JSON",
      });
    }
  });

export const organisationBaseSchema = z.object({
  ...
  mapGeoJson: mapGeoJsonField,
});

export const regionSchema = organisationBaseSchema.extend({
  code: z.string().min(1, "Code is required"),
  headquarters: z.string().min(1, "Headquarters is required"),
});

export const circleSchema = organisationBaseSchema.extend({
  regionId: z.string().min(1, "Region is required"),
});

export const divisionSchema = organisationBaseSchema.extend({
